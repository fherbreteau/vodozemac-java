...
use syn::{parse_macro_input, ItemFn, FnArg, Pat, Type, TypeReference, Error, PatType, TypeSlice, PatIdent, ReturnType, TypePath, GenericArgument, TypeArray, PathArguments};
                ...
                let arg_type = match segment.ident.to_string().as_str() {
                    "i8" | "u8" => {
                        if mutability.is_some() {
                            ...
                            pre_statements.push(quote! {
                                let #native_ident = if #ident.as_raw().is_null() {
                                    None
                                } else {
                                    Some(crate::get_byte_array_region_const::<#len>(env, &#ident, 0)?)
                                };
                            });
                            jni_call_args.push(quote! { #native_ident.as_ref() });
                        } else {
                        ...
                        if mutability.is_some() {
                            ...
                            pre_statements.push(quote! {
                                let #native_ident = if #ident.as_raw().is_null() {
                                    ...
                                    Some(crate::get_short_array_region_const::<#len>(env, &#ident, 0)?)
                                };
                            });
                            jni_call_args.push(quote! { #native_ident.as_ref() });
                        } else {
                        ...
                        if mutability.is_some() {
                            ...
                            pre_statements.push(quote! {
                                let #native_ident = if #ident.as_raw().is_null() {
                                    ...
                                    Some(crate::get_int_array_region_const::<#len>(env, &#ident, 0)?)
                                };
                            });
                            jni_call_args.push(quote! { #native_ident.as_ref() });
                        } else {
