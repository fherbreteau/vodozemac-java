...
`PkEncryption`, `PkDecryption`) are safe to use from multiple threads. Methods
that access the native state are synchronized on the instance, and `close()`
can never race with an ongoing native call. Sharing a single handle across
threads serializes the native calls on that object; distinct handles can be
used concurrently without any locking.
