...
[![API docs](https://img.shields.io/badge/Javadoc-apidocs%2Flatest-41B096.svg)](https://fherbreteau.github.io/vodozemac-java/apidocs/latest/index.html)
...
📚 **[Javadoc](https://fherbreteau.github.io/vodozemac-java/apidocs/latest/index.html)**
...
`Ed25519KeyPair`, `PkEncryption`, `PkDecryption`) are safe to use from multiple
threads. Methods that access the native state are synchronized on the
instance, and `close()` can never race with an ongoing native call. Sharing a
single handle across threads serializes the native calls on that object;
