...
use vodozemac::{base64_decode, base64_encode, PickleError};
...
pub extern "system" fn JNI_OnLoad(vm: jni::JavaVM, _: *mut c_void) -> jint {
    fn inner(vm: jni::JavaVM) -> jni::errors::Result<()> {
        ...
        let class_name = |suffix: &str| format!("com/oodrive/matrix/vodozemac/bindings/{suffix}");

        let megolm_classes = megolm::MegolmJniClasses {
            group_session: &env.find_class(class_name("megolm/GroupSessionBindings"))?,
            session_config: &env.find_class(class_name("megolm/SessionConfigBindings"))?,
            inbound_group_session: &env
                .find_class(class_name("megolm/InboundGroupSessionBindings"))?,
            message: &env.find_class(class_name("megolm/MessageBindings"))?,
            session_key: &env.find_class(class_name("megolm/SessionKeyBindings"))?,
            exported_session_key: &env
                .find_class(class_name("megolm/ExportedSessionKeyBindings"))?,
        };

        let sas_classes = sas::SasJniClasses {
            established_sas: &env.find_class(class_name("sas/EstablishedSasBindings"))?,
            mac: &env.find_class(class_name("sas/MacBindings"))?,
            sas: &env.find_class(class_name("sas/SasBindings"))?,
            sas_bytes: &env.find_class(class_name("sas/SasBytesBindings"))?,
        };

        let olm_classes = olm::OlmJniClasses {
            account: &env.find_class(class_name("olm/AccountBindings"))?,
            message: &env.find_class(class_name("olm/MessageBindings"))?,
            session: &env.find_class(class_name("olm/SessionBindings"))?,
            session_config: &env.find_class(class_name("olm/SessionConfigBindings"))?,
            session_keys: &env.find_class(class_name("olm/SessionKeysBindings"))?,
        };

        let pk_encryption_classes = pk_encryption::PkEncryptionJniClasses {
            pk_encryption: &env.find_class(class_name("pkencryption/PkEncryptionBindings"))?,
            pk_decryption: &env.find_class(class_name("pkencryption/PkDecryptionBindings"))?,
        };

        let key_class = &env.find_class(class_name("KeyBindings"))?;
        ...
        pk_encryption::register_jni(&mut env, &pk_encryption_classes)?;

        keys::register_jni(&mut env, key_class)?;

        let slice_bindings = env.find_class(class_name("SliceBindings"))?;
...
trait Pickle : Sized {
    fn from_unencrypted(base64: &str) -> Result<Self, PickleError>;
    fn to_unencrypted(self) -> String;
}
...
{
    fn from_unencrypted(base64: &str) -> Result<Self, PickleError> {
        let value = base64_decode(base64)?;
        Ok(serde_json::from_slice(&value)?)
    }

    fn to_unencrypted(self) -> String {
        let value = serde_json::to_vec(&self).expect("it to be serializable");
        base64_encode(&value)
    }
}
...
macro_rules! unpickle {
    ($ciphertext:path, $key:path, $pickle:path, $real:path, $mapper:path) => {
        {
            use $crate::Pickle;

            let ciphertext = core::str::from_utf8($ciphertext).expect("valid utf8");
            let pickle = match $key {
                Some(key) => <$pickle>::from_encrypted(ciphertext, key),
                None =>  <$pickle>::from_unencrypted(ciphertext),
            };
            pickle
                .map(<$real>::from_pickle)
                .map($mapper)
                .map(boxed)
                .map_err(Into::into)
                .into()
...
#[macro_export]
macro_rules! pickle {
    ($value:path, $key:path, $mapper:path) => {
        {
            use $crate::Pickle;

            let value = $mapper($value);
            let pickle = value.pickle();
            match $key {
                Some(key) => pickle.encrypt(key),
                None => pickle.to_unencrypted()
            }.into()
        }
    };
}