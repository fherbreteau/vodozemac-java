...
use crate::{AsUsize, CResult, Either, boxed, free, unpickle, pickle};
use jni::JNIEnv;
...
pub fn register_jni(env: &mut JNIEnv, class: &JClass) -> jni::errors::Result<()> {
    env.register_native_methods(
        ...
        &[
            ...
            VODOZEMAC_OLM_SESSION_FROM_LIBOLM_PICKLE_JNI.into(),
        ],
    )
}
...
#[ffi]
#[sret]
pub fn vodozemac_olm_session_pickle(session: &Session, pickle_key: Option<&[u8; 32]>) -> CSlice<u8> {
    pickle!(session, pickle_key, RwLock::read)
}
...
pub fn vodozemac_olm_session_from_pickle(
    ...
    unpickle!(ciphertext, pickle_key, SessionPickle, olm::Session, RwLock::new)
}

#[ffi]
#[sret]
pub fn vodozemac_olm_session_from_libolm_pickle(
    #[expand] pickle: &[u8],
    #[expand] pickle_key: &[u8],
) -> CResult<NonNull<Session>, CErrorStr> {
    let pickle = str::from_utf8(pickle).expect("valid utf8");

    olm::Session::from_libolm_pickle(pickle, pickle_key)
        .map(RwLock::new)
        .map(boxed)
        .map_err(Into::into)
        .into()
