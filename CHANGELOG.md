...
- **Rust in SonarCloud**: The Sonar analysis now covers all code — `sonar.sources` includes `rust/src` (Java + Rust), the Rust coverage is generated with `cargo llvm-cov --lcov` in the quality job (`sonar.rust.lcov.reportPaths`), and the Cargo manifest is declared (`sonar.rust.cargo.manifestPath`) so Rust files are analyzed alongside Java on SonarCloud (#91) (#91)
